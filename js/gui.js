...
function CheckResult() {
	if(GameBoard.fiftyMove >= 100) {
		 $("#GameStatus").text("Oyun berabere {elli hamle kuralı}"); 
		 return BOOL.TRUE;
	}
	
	if (ThreeFoldRep() >= 2) {
     	$("#GameStatus").text("Oyun berabere {3 hamle tekrarı}"); 
     	return BOOL.TRUE;
    }
	
	if (DrawMaterial() == BOOL.TRUE) {
     	$("#GameStatus").text("Oyun berabere {imkansız oyun}"); 
     	return BOOL.TRUE;
    }
	...
	if(InCheck == BOOL.TRUE) {
		...
		$("#GameStatus").text("Oyun berabere {pat}");return BOOL.TRUE;
	}
