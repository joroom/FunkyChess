...
function UpdateDOMStats(dom_score, dom_depth) {
	...
	if(Math.abs(dom_score) > MATE - MAXDEPTH) {
		scoreText = "Skor: Hamlede " + (MATE - (Math.abs(dom_score))-1) + " mat";
	}
	
	
	$("#DepthOut").text("Derinlik: " + dom_depth);
	$("#ScoreOut").text(scoreText);
	$("#TimeOut").text("Zaman: " + (($.now()-SearchController.start)/1000).toFixed(1) + "s");
	$("#BestOut").text("En iyi hamle: " + PrMove(SearchController.best));
}
